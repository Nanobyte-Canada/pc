...
export default function PortfolioSummaryWidget(_props: { connectionId?: number }) {
  ...
  return (
    <div className="ps-widget">
      {/* Mobile greeting hero — hidden on desktop */}
      <div className="ps-hero-mobile">
        <div className="ps-hero-mobile-left">
          <div className="ps-hero-greeting">{getGreeting()}</div>
          <div className="ps-hero-total">C$ {fmtNum(totalValue)}</div>
          <div className="ps-hero-pill-row">
            {dayChangeAvailable ? (
              <span className={`ps-hero-pill ${isPositive ? 'ps-hero-pill-gain' : 'ps-hero-pill-loss'}`}>
                {isPositive
        ...
        <div className="ps-hero-avatar">{initials}</div>
      </div>

      {/* KPI cards row */}
      <div className="ps-kpi-row">
        {/* Investment card */}
        <div className="ps-kpi-card ps-stat">
