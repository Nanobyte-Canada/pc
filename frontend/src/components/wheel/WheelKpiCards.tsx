...
/**
 * Displays 5 KPI summary cards for the Wheel Strategy dashboard.
 *
 * Shows Capital Available (USD), Capital Deployed, CC Available,
 * Premium & P&L, and Position counts in a simplified single-value format.
 */
export function WheelKpiCards({ metrics, buyingPower, ccEligible, positionCounts, fxRate }: WheelKpiCardsProps) {
  const bpCad = buyingPower.find(bp => bp.currency === 'CAD')?.amount ?? 0
  const bpUsd = buyingPower.find(bp => bp.currency === 'USD')?.amount ?? 0
  const totalAvailableUsd = bpCad / fxRate + bpUsd
  const totalCC = Array.from(ccEligible.values()).reduce((sum, cc) => sum + cc.contractsAvailable, 0)

  const totalDeployedUsd = (metrics?.deployedCsp.usd ?? 0) + (metrics?.ccsWritten.usd ?? 0)
    ...
    <div className="wheel-kpi-row">
      <div className="wheel-kpi-card">
        ...
        <div className="wheel-kpi__value">
          {formatCurrency(totalAvailableUsd, 'USD')}
        </div>
      </div>
      ...
      <div className="wheel-kpi-card">
        ...
        <div className="wheel-kpi__value">
          {formatCurrency(totalDeployedUsd, 'USD')}
        </div>
      </div>
