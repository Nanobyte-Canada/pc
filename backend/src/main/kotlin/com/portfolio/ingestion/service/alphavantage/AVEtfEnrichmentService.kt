...
/**
 * Service for enriching ETF entities from stored Alpha Vantage raw payloads.
 *
 * This service reads from the avRawPayload field (which was populated by AVEtfIngestionService)
 ...
 *
 * Enrichment includes:
 * - ETF overview (expense ratio, AUM, inception date, etc.)
 * - Sector allocations (11 GICS sectors)
 * - Top holdings
 *
 ...
 */
@Service
class AVEtfEnrichmentService(
    private val config: IngestionConfig,
    private val etfRepository: EtfRepository,
    private val batchProcessor: AVEtfEnrichmentBatchProcessor,
    private val trackingService: IngestionTrackingService
) {
    private val log = LoggerFactory.getLogger(javaClass)

    /**
     * Enriches ETFs from stored Alpha Vantage raw payloads.
     *
    ...
    fun enrichEtfs(step: IngestionStep): StepResult {
        ...
        log.info("Starting AV ETF enrichment from stored payloads")
        ...
        do {
            ...
            val batch = etfRepository.findAvEnrichmentCandidatesWithIngestionSuccess(
            ...
            if (batch.isEmpty()) {
                if (totalBatches == 0) {
                    log.info("No ETFs pending Alpha Vantage enrichment with ingested data")
                }
                break
            }

            totalBatches++
            log.info("Processing batch {} with {} ETFs for AV enrichment", totalBatches, batch.size)
        ...
        log.info("AV ETF enrichment complete: total_batches={}, processed={}, updated={}, failed={}",
