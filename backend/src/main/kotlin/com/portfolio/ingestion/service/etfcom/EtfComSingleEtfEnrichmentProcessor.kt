...
class EtfComSingleEtfEnrichmentProcessor(
    ...
    /**
     * Process a single ETF in its own transaction. If this fails, only this ETF's
     * transaction rolls back — other ETFs in the batch are unaffected.
    ...
    private fun processHoldings(etf: Etf, data: EtfComEnrichmentData, step: IngestionStep) {
        ...
        entityManager.flush() // Ensure delete is executed before inserts

        // Build all holdings first, then deduplicate by (stock_id, asOfDate) before saving
        data class HoldingKey(val stockId: Long?, val rawTicker: String?)

        val holdingsList = mutableListOf<Pair<HoldingKey, EtfHolding>>()
        ...
        for ((rank, holding) in holdings.withIndex()) {
            ...
            // Key: use stock_id for resolved holdings, rawTicker for unresolved (null stock_id won't violate the constraint)
            val key = if (stock != null) HoldingKey(stock.id, null) else HoldingKey(null, symbol)
            ...
            holdingsList.add(key to newHolding)
        }

        // Deduplicate: group by key, sum weights for resolved duplicates
        val deduplicated = holdingsList
            .groupBy { it.first }
            .map { (key, entries) ->
                val first = entries.first().second
                if (entries.size > 1 && key.stockId != null) {
                    val summedWeight = entries.fold(BigDecimal.ZERO) { acc, (_, h) -> acc + (h.weight ?: BigDecimal.ZERO) }
                    log.debug("Deduplicated {} holdings for ETF {} stock_id={} (raw tickers: {}), summed weight: {}",
                        entries.size, etf.symbol, key.stockId,
                        entries.map { it.second.rawTicker }.joinToString(", "), summedWeight)
                    first.weight = summedWeight
                    first.etfcomWeight = summedWeight
                }
                first
            }

        for (holding in deduplicated) {
            etfHoldingRepository.save(holding)
            holdingsCreated.increment()
        }

        log.debug("Processed {} holdings for ETF {} ({} before dedup)", deduplicated.size, etf.symbol, holdingsList.size)
    }
