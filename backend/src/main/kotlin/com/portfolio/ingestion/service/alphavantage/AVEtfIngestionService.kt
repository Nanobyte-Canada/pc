...
/**
 * Service for ingesting raw Alpha Vantage data for ETF entities.
 *
 * This service only fetches data from the Alpha Vantage ETF_PROFILE endpoint
 * and stores the raw JSON payload. It does NOT parse or map fields to entity columns.
 *
 * The enrichment step (AVEtfEnrichmentService) is responsible for parsing
 ...
 */
@Service
class AVEtfIngestionService(
    private val config: IngestionConfig,
    private val avClient: AlphaVantageClient,
    private val etfRepository: EtfRepository,
    private val batchProcessor: AVEtfIngestionBatchProcessor,
    ...
    /**
     * Ingests raw Alpha Vantage data for ETFs.
     *
     * Fetches from AV ETF_PROFILE endpoint and stores raw JSON only.
     ...
     */
    fun ingestEtfs(step: IngestionStep): StepResult {
        var processed = 0
        var updated = 0
        var failed = 0
        ...
        if (remainingQuota <= 0) {
            log.warn("Daily API quota exhausted. Skipping ETF ingestion.")
            ...
            return StepResult(
                ...
                updated = 0,
                failed = 0,
                metadata = mapOf("type" to "etfs", "skipped" to "quota_exhausted")
            )
        }

        log.info("Starting AV ETF ingestion. Remaining daily quota: {}", remainingQuota)
        ...
        do {
            ...
            if (remainingQuota <= 0) {
                log.info("Daily quota exhausted. Stopping ETF ingestion.")
                break
            }

            // Fetch next batch of candidates
            val batch = etfRepository.findAvIngestionCandidates(
                statuses = statuses,
            ...
            if (batch.isEmpty()) {
                if (totalBatches == 0) {
                    log.info("No ETFs pending Alpha Vantage ingestion")
                }
                break
            }

            totalBatches++
            log.info("Processing batch {} with {} ETFs for AV ingestion", totalBatches, batch.size)
        ...
        log.info("AV ETF ingestion complete: total_batches={}, processed={}, updated={}, failed={}",
