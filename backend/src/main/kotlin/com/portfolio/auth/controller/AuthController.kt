...
class AuthController(
    ...
    fun handleEmailExists(ex: EmailAlreadyExistsException): ResponseEntity<AuthErrorResponse> {
        return ResponseEntity.status(HttpStatus.CONFLICT)
            .body(AuthErrorResponse(error = ex.errorCode, message = ex.message, field = "email"))
    }

    @ExceptionHandler(InvalidCredentialsException::class)
    fun handleInvalidCredentials(ex: InvalidCredentialsException): ResponseEntity<AuthErrorResponse> {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
            .body(AuthErrorResponse(error = ex.errorCode, message = ex.message))
    }

    @ExceptionHandler(AccountLockedException::class)
    fun handleAccountLocked(ex: AccountLockedException): ResponseEntity<AuthErrorResponse> {
    ...
    fun handleEmailNotVerified(ex: EmailNotVerifiedException): ResponseEntity<AuthErrorResponse> {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
            .body(AuthErrorResponse(error = ex.errorCode, message = ex.message))
    }

    @ExceptionHandler(InvalidTokenException::class)
    fun handleInvalidToken(ex: InvalidTokenException): ResponseEntity<AuthErrorResponse> {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
            .body(AuthErrorResponse(error = ex.errorCode, message = ex.message))
    }

    @ExceptionHandler(InvalidPasswordException::class)
    fun handleInvalidPassword(ex: InvalidPasswordException): ResponseEntity<AuthErrorResponse> {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
            .body(AuthErrorResponse(error = ex.errorCode, message = ex.message, field = "password"))
    }

    @ExceptionHandler(UserNotFoundException::class)
    fun handleUserNotFound(ex: UserNotFoundException): ResponseEntity<AuthErrorResponse> {
