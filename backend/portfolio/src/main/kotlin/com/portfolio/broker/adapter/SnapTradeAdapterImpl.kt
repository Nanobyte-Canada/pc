...
class SnapTradeAdapterImpl(
    ...
    override fun getLoginRedirectUrl(
        ...
        return try {
            ...
            request.connectionType(connectionType ?: "trade-if-available")
    ...
    override fun listBrokerageAuthorizationTypes(brokerageSlug: String?): List<SnapTradeBrokerageAuthTypeDto> {
        return try {
            val request = snaptrade.referenceData.listAllBrokerageAuthorizationType()
            if (brokerageSlug != null) request.brokerage(brokerageSlug)
            request.execute().map { authType ->
                SnapTradeBrokerageAuthTypeDto(
                    id = authType.id,
                    type = authType.type?.value,
                    authType = authType.authType?.value,
                    brokerageId = authType.brokerage?.id
                )
            }
        } catch (e: ApiException) {
            throw mapApiException(e)
        ...
        return try {
            ...
            response.map { order ->
                SnapTradeAccountOrderDto(
                    ...
                    timePlaced = order.timePlaced?.toString(),
                    timeUpdated = order.timeUpdated?.toString(),
                    timeExecuted = order.timeExecuted?.toString(),
    ...
    private fun Brokerage.toDto() = SnapTradeBrokerageDto(
        ...
        isDegraded = null, // Field not available in SDK v5.0.168
        allowsTrading = allowsTrading,
        allowsFractionalUnits = allowsFractionalUnits,
        hasReporting = hasReporting,
        isRealTimeConnection = isRealTimeConnection,
        brokerageType = brokerageType?.let { BrokerageTypeDto(it.id, it.name) }
    )
