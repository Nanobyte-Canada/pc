...
class DriftCalculationService(
    ...
    fun getRebalanceProgress(connection: BrokerConnection): RebalanceProgressDto {
        ...
        // FX-convert all position values to CAD
        val fxRates = mutableMapOf<String, BigDecimal>()
        val totalValueCAD = positions.sumOf { pos ->
            val rate = fxRates.getOrPut(pos.currency) {
                exchangeRateService.getRate(pos.currency, today) ?: BigDecimal.ONE
            }
            (pos.currentValue ?: BigDecimal.ZERO) * rate
        }

        val modelSymbols = model.allocations.map { it.symbol.uppercase() }.toSet()
        val entries = mutableListOf<RebalanceProgressEntry>()

        // Model allocation entries (target > 0)
        for (alloc in model.allocations) {
            val matchingPositions = positions.filter { it.symbol.equals(alloc.symbol, ignoreCase = true) }
            val actualValueCAD = matchingPositions.sumOf { pos ->
                val rate = fxRates.getOrPut(pos.currency) {
                    exchangeRateService.getRate(pos.currency, today) ?: BigDecimal.ONE
                }
                (pos.currentValue ?: BigDecimal.ZERO) * rate
            }
            val actualPercent = if (totalValueCAD > BigDecimal.ZERO)
                actualValueCAD.divide(totalValueCAD, 4, RoundingMode.HALF_UP).multiply(BigDecimal(100))
            else BigDecimal.ZERO

            entries.add(RebalanceProgressEntry(
                symbol = alloc.symbol,
                securityName = matchingPositions.firstOrNull()?.securityName,
        ...
        // Non-model positions (target = 0, will be sold)
        for (pos in positions) {
            if (!modelSymbols.contains(pos.symbol.uppercase())) {
                val rate = fxRates.getOrPut(pos.currency) {
                    exchangeRateService.getRate(pos.currency, today) ?: BigDecimal.ONE
                }
                val valueCAD = (pos.currentValue ?: BigDecimal.ZERO) * rate
                val actualPercent = if (totalValueCAD > BigDecimal.ZERO)
                    valueCAD.divide(totalValueCAD, 4, RoundingMode.HALF_UP).multiply(BigDecimal(100))
                else BigDecimal.ZERO
        ...
        val accuracy = calculateModelAccuracy(entries.filter { !it.isNonModel })
    ...
    private fun calculateModelAccuracy(entries: List<RebalanceProgressEntry>): BigDecimal {
        if (entries.isEmpty()) return BigDecimal.ZERO
        val totalDrift = entries.sumOf { (it.targetPercent - it.actualPercent).abs() }
        val maxDrift = BigDecimal(200)
        return BigDecimal(100).subtract(
            totalDrift.divide(maxDrift, 4, RoundingMode.HALF_UP).multiply(BigDecimal(100))
