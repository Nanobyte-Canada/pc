...
| `WheelKpiCards.tsx` + `.css` | 5 KPI cards with simplified single-value layout. Capital Available (USD total), Capital Deployed (USD total), CC Available (total contracts), Premium & P&L (USD), Positions (total with CSP/CC breakdown). Desktop: 5-column grid. Mobile: 2×2 + 1 full-width row |
| `PositionCard.tsx` + `.css` | Compact position card: CSP = indigo (var(--csp-*)), CC = orange (var(--cc-*)). Row 1: strike + type label (Put/CC) + OTM%. Row 2: P&L (signed). Row 3 (conditional): collected premium. No account badge |
