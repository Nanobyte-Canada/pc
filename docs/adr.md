...
## ADR-0037: Durable per-chunk broker sync with honest status and single-flight guard

**Status:** Accepted | **Date:** 2026-09-24

**Context:** Phase 2 of broker sync correctness (spec `docs/superpowers/specs/2026-09-22-broker-sync-correctness-design.md`, plan `docs/superpowers/plans/2026-09-24-broker-sync-correctness-phase-2.md`). The full-history activity sync ran every chunk inside a single `REQUIRES_NEW` transaction: a failure escaping the loop rolled back years of already-fetched chunks, chunk errors that were swallowed still let the run finish and advance `last_activities_fetched_at`, so the watermark claimed a complete sync when it was not, and there was no persisted resume point — the next run always restarted from the lookback boundary. Nothing recorded the outcome of a sync (activities or balances) either way. The same connection could be worked concurrently by a scheduled run, a manual fetch, `/sync-all`, and the 60s `OrderStatusSyncScheduler` tick. The broker-sync cron expressions were evaluated in the JVM default zone (UTC in the container), so the configured 4:20 PM / 6:00 AM never fired at ET wall-clock. Finally, `broker.sync.max-lookback-years` defaulted to 30 while Questrade retains only ~16 months of activity, so first syncs spent most chunks fetching empty ranges.

**Decision:**
- **Durable per-chunk progress (V78).** `broker_sync_progress` keyed by `(connection_id, sync_kind)` stores `next_chunk_end`, the next (earlier) chunk boundary for the backward-walking full-history sync. Each chunk commits its fetched activities *and* the advanced progress row in one transaction, with the gateway HTTP call outside it — replacing the single full-history transaction. A mid-run failure costs at most one chunk; the next run resumes from `next_chunk_end`.
- **Honest watermark/status columns (V79).** `broker_connections` gains `last_activities_sync_status` and `last_balance_sync_status` (`SUCCESS` / `PARTIAL` / `FAILED`). The `last_*_fetched_at` watermarks advance only on success, so a watermark means the data was actually synced, not merely attempted.
- **In-process single-flight.** `ConnectionSyncGuard` (per-`connection_id` `ReentrantLock`, re-entry rejected even for the holding thread) gates all four sync entry points: manual position fetch (also the positions step of `/sync-all`), activity sync (full-history and incremental), balance sync, and `OrderStatusSyncScheduler`'s per-connection order sync (human-approved scope). The guard is deliberately in-process only; ShedLock (or any clustered lock) is deferred until the portfolio runs more than one replica and is noted as such in code comments rather than added as a dependency.
- **ET scheduler zones.** Both `AccountDataSyncScheduler` crons (`broker.sync.cron`, `broker.sync.cron-morning`) declare `zone = "America/Toronto"`, making cron strings ET wall-clock regardless of JVM/container timezone. `RebalanceScheduler` is explicitly out of scope and keeps the JVM default zone.
- **Bounded lookback.** `broker.sync.max-lookback-years` defaults to 5 (was 30), reflecting Questrade's ~16-month activity retention (human decision 2026-09-24).

**Consequences:** An interrupted or failed first sync resumes where it stopped instead of re-fetching the whole history, and users can tell a partial sync from a complete one without inferring it from a timestamp. Status columns are additive and nullable, so pre-existing connections read as "unknown" until their next sync. Single-flight is per JVM: with today's single-replica deployment overlapping runs for one connection are impossible, but adding a replica reintroduces cross-instance overlap — a clustered lock must ship with that change. Cron configuration now depends on the declared ET zone (server timezone changes no longer shift sync times), while `RebalanceScheduler` remains JVM-zone by deliberate omission. The 5-year lookback never backfills activity older than five years on a first sync; changing `BROKER_SYNC_MAX_LOOKBACK_YEARS` later does not retroactively extend already-completed syncs. V78/V79 are additive Flyway migrations with no data backfill and no downtime.
