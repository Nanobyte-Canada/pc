...
| `backend/src/main/kotlin/com/portfolio/dto/response/DataQualityEnvelope.kt` | Response wrapper with data quality metadata |
...
- [ ] **Step 5: Remove local @ExceptionHandler methods from BrokerController**

In `BrokerController.kt`, remove the local `@ExceptionHandler` methods (lines ~287-306) that handle `IllegalArgumentException` and `IllegalStateException` — the global handler now covers these.

Check other controllers for local `@ExceptionHandler` methods and remove them if the global handler covers the same exception types.
...
Run: `docker compose up -d redis && docker compose exec backend ./gradlew test --tests "*.ApiIntegrationTest"`
Expected: Tests pass (Redis available via docker-compose)
...
### Task 9: Add @Cacheable to LookThroughService

**Files:**
- Modify: `backend/src/main/kotlin/com/portfolio/service/LookThroughService.kt`

- [ ] **Step 1: Read current LookThroughService**

Read the full file to understand `computeLookThroughWithQuality()`.

- [ ] **Step 2: Add @Cacheable to sector allocation lookups**

The `sectorAllocationFactsetRepository.findLatestByEtfId(etf.id)` call (line ~402) is a good caching candidate. Add `@Cacheable` to the repository method or create a cached wrapper:

In `LookThroughService.kt`, extract sector allocation lookup into a cacheable method:

```kotlin
@Cacheable(value = ["etf-sector-allocations"], key = "#etfId")
fun getCachedSectorAllocations(etfId: Long): List<SectorAllocationFactset> {
    return sectorAllocationFactsetRepository.findLatestByEtfId(etfId)
}
```

Replace direct repository calls with `getCachedSectorAllocations(etf.id)`.

**Note:** The main `computeLookThroughWithQuality` method itself is harder to cache because its input is a `List<PortfolioPositionRequest>` which doesn't have a natural cache key. Focus caching on the repository-level lookups it makes.
...
```bash
git add backend/src/main/kotlin/com/portfolio/service/LookThroughService.kt
git commit -m "feat: add Redis caching for ETF sector allocation lookups"
```
...
- [ ] **Step 1: Create the DataQualityEnvelope**

```kotlin
package com.portfolio.dto.response

data class DataQualityEnvelope<T>(
    val data: T,
...
- [ ] **Step 2: Update silent catch in calculateHoldingsCount (lines ~158-178)**
...
- [ ] **Step 3: Apply same pattern to getSectorExposure (lines ~250-255)**
...
- [ ] **Step 4: Apply same pattern to getGeographyExposure (lines ~332-337)**
...
- [ ] **Step 5: Add `warnings` field to relevant response DTOs**
...
```bash
git add backend/src/main/kotlin/com/portfolio/dto/response/DataQualityEnvelope.kt \
