...
- The frontend Wheel page queries broker positions directly, not the strategy service
- **DB schema is significantly ahead of implementation**
...
- `snaptrade_order_id` column still present after Snaptrade removal
...
`ProviderRegistry` suggests multiple providers (AlphaVantage, etc.) but only `EodhdProvider` is implemented.
...
5 Dockerfiles with ~90% identical content.

### 4.4 Spring Profile Duplication

5 services × 2 profiles (uat + prod) = 10 nearly identical files.
...
`useChartTheme` and `useAgGridTheme` both wrap `useThemeStore`.
