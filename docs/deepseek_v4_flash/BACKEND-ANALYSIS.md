...
- **`broker-gateway` declares dependency but never uses it** — `build.gradle.kts` has `implementation("com.portfolio:common")` but no imports from `com.portfolio.common`
- **`strategy` declares dependency but never uses it** — No imports from `com.portfolio.common` found

### Redundancies
- **`portfolio` service has its own domain model** that partially overlaps with `common` (e.g., `Quote`, `Greeks` concepts), resulting in parallel type hierarchies
...
### Disconnected Code
- The broker subsystem is very large (80+ files, 17 services) — handles portfolio groups, trading, dashboard, performance, notifications, and exchange rates all under `broker/`, mixing domains
...
4. **Stale data threshold** — `stale-threshold-days: 7` means data older than 7 days is re-fetched
...
5. **Estimated IV hardcoded** — `ChainController` has `ESTIMATED_IV = 0.30` used as fallback
...
7. **Calculator limitations** — Only delta computed (gamma, theta, vega = zero)
8. **Strategy orders/positions tables unused** — `strategy.orders`, `strategy.order_legs`, `strategy.executions`, `strategy.positions`, `strategy.position_legs` tables have no repositories
9. **`strategy.admin_actions` table unused** — No corresponding service or controller
...
5. **Shared database with portfolio** — `broker_gateway` schema in same PostgreSQL creates tight coupling
6. **IBKR adapter initializes connection in constructor** — Side effect in `init {}` block
...
- Snaptrade removed from portfolio via migrations V72 and V73
- Strategy schema still references it

### 8.5 AlphaVantage / SeekingAlpha Migration Remnants

- V26__add_av_ingestion_step_names.sql — AlphaVantage step names
- V18__alpha_vantage_enrichment.sql
- V21__drop_seekingalpha_columns.sql
- V13__seekingalpha_enrichment_tracking.sql
- These providers removed but migration traces remain