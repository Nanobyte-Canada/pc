...
| 2 | `strategy` | `spring-boot-starter-data-redis` | Redis configured but no code uses it |
...
The `strategy` service has 9 Flyway-created tables for Wheel (wheel_accounts, wheel_configs, wheel_recommendations, wheel_holdings, etc.) but the `StrategyType` enum has no `WHEEL` entry. The frontend has a complete Wheel page but it queries broker positions directly, not the strategy service. **DB schema is ahead of implementation.**

### 3.2 Strategy Calculator: No market data integration

`StrategyCalculator.calculate()` requires `spotPrice` in the request. It does not fetch live quotes from market-data service. The frontend provides spot price from its own store.

### 3.3 Broker Gateway: Standalone but coupled

- Has its own DB schema in the **same PostgreSQL** as portfolio service
- Has no API key validation despite configured `GATEWAY_API_KEY`
- Separate process but shared database = hybrid architecture

### 3.4 Common Library: Underutilized

`common` has canonical domain objects (`Quote`, `Greeks`, `OptionsChain`) but the **portfolio service** (largest service) doesn't use `common` at all.
...
|---|-----|----------|---------|
| 1 | No API rate limiting | **High** | All services |
| 2 | No MFA | **Medium** | Auth system |
| 3 | Hardcoded Wealthsimple client ID | **High** | broker-gateway |
| 4 | API key not validated on gateway | **High** | broker-gateway |
...
| 9 | Fix unused common dependency in broker-gateway | Low | Build |
