...
**Consumers:** market-data (heavy), strategy (moderate), broker-gateway (declared, zero usage)
...
- Wheel strategy DB schema (tables exist, no implementation code)
...
|-------|-----------|---------|
| `authStore` | Yes | User, JWT, session |
| `quoteStore` | No | Real-time quotes, chains |
| `strategyStore` | No | Options strategy state |
| `toastStore` | No | Toast notifications |
| `themeStore` | Yes | Dark/light mode |
| `sidebarStore` | Yes | **DEAD — never imported** |
| `portfolioStore` | No | Portfolio builder state |
| `analysisStore` | No | Analysis results |
