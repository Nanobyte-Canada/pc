...
| portfolio | ingestion | HTTP REST | None | Health check only (`IngestionServiceHealthIndicator`) |
| strategy | market-data | Configured URL | None | Quote/chain data (configured but not implemented) |
| strategy | portfolio | Configured URL | None | Portfolio data (configured but not implemented) |
...
```
VPS Server
├── Cloudflare Tunnel → public internet (portfolio.nanobyte.ca)
