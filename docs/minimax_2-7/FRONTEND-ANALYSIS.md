...
### 4.2 Unused UI Primitives (Defined but Never Imported)
...
### 5.4 Inconsistent API Methods

Three different API methods used across services:
- `apiFetch` — authenticated fetch with CSRF
- `proxyFetch` — unauthenticated fetch for proxied services
- Raw `fetch()` — used in `adminService.ts`, missing CSRF handling

### 5.5 Duplicate Theme Hooks

`useChartTheme` and `useAgGridTheme` both just wrap `useThemeStore` — redundant wrappers.

---

## 6. Disconnected Modules

### 6.1 AnalyticsPage Disconnected from Store
...
- `usePortfolioAnalysis` hook just wraps a query/mutation but doesn't connect to the store
- The page appears non-functional — data flows nowhere

### 6.2 AdminService Uses Raw Fetch

`adminService.ts` uses raw `fetch()` instead of `apiFetch()`, missing:
- CSRF token handling
- JWT cookie handling
- Authentication state checks

### 6.3 WheelPage Direct Broker Queries

The frontend Wheel page (`WheelPage.tsx`) queries broker positions directly via `brokerService.ts` rather than through the `strategy` service, which has wheel-related database tables but no implementation code.
...
- `marketHolidays.ts` — Hardcoded holidays, needs annual updates
- FX rate default of 1.38 hardcoded in multiple places instead of using `useExchangeRate`
- `useWheelPositions.ts` line 278: `resp.positions as unknown as BrokerPosition[]` — unsafe cast